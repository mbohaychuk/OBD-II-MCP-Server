...
@pytest.mark.asyncio
async def test_read_dtcs_generic_range_code_keeps_sae_definition_over_make() -> None:
    """P0420 is a generic-range (P0) code: the canonical SAE definition must win
    even when a make is supplied — passing make must NOT degrade it to a
    manufacturer's elaboration of the same code."""
    stub = _StubClient({"GET_DTC": [("P0420", "")]})
    with DtcDatabase(DEFAULT_DB_PATH) as db:
        result = await read_dtcs(  # type: ignore[arg-type]
            stub, scope="stored", dtc_db=db, manufacturer="Audi"
        )
    assert result["codes"][0]["description"] == (
        "Catalyst System Efficiency Below Threshold Bank 1"
    )
...
@pytest.mark.asyncio
async def test_read_dtcs_manufacturer_range_code_uses_make_via_full_marque_alias() -> None:
    """P1450 is a manufacturer-range (P1) code that means different things per
    make. Passing the full NHTSA marque 'Chevrolet' (aliased to the DB's
    'CHEVY') must surface Chevy's definition, not the unrelated generic row."""
    stub = _StubClient({"GET_DTC": [("P1450", "")]})
    with DtcDatabase(DEFAULT_DB_PATH) as db:
        result = await read_dtcs(  # type: ignore[arg-type]
            stub, scope="stored", dtc_db=db, manufacturer="Chevrolet"
        )
    assert result["codes"][0]["description"] == "Barometric Pressure Sensor Circuit"
    assert result["codes"][0]["source"] == "manufacturer"
